# Himnario ASD (v2.1.0)

## Descripción
- Aplicación para proyectar los 613 himnos en un formato de video de una manera fácil y rápida en un computadora o laptop.
- Puedes crear listas de reproducción para que la música siga reproduciéndose sin interrupciones.
- Una aplicación ligera con un diseño minimalista, que le permite una acompañar sus servicios en la iglesia.
<br/>

## Características
Esta aplicación del himnario adventista incluye los siguiente:
- Cuenta con los de los _613_ himnos del **nuevo himnario adventista** _(ed. 2011)_
- Contenido multimedia de los himnos en los formatos de:
    - **video** con letra en sincronía, audio cantado y audio de pista musical
    - **imágenes** con letra del himno
- Funciones de búsqueda de himnos por _título_ o _número_
- Funcionalidad para crear listas de reproducción de himnos
- Controles de reproducción para:
    - Cambio de audio _cantado_ o solo _música_
    - Estado de reproducción
    - Control para _mutear_ el audio
    - Control para reproducción automática
    - Controles para _avanzar_ o _retroceder_ 10 segundos
    - Controles para cambiar la letra del himno
    - Controles para administrar la lista de reproducción en tiempo real
- Atajos del teclado activar pantalla completa
- El título de la ventana cambia respecto al himno que se reproduce
<br/>

### Accesos directos del teclado
- La app en la vista de búsqueda de himnos cuenta con los siguientes accesos directos de teclado:
    - `ArrowDown | ArrowUp`: _Navega_ entre los resultados de búsqueda de himnos
    - `Escape`: _Regresa el control_ a la barra de búsqueda o _borra el texto_ ingresado
    - `Enter`: Cuando un resultado de búsqueda este subrayado con azul _iniciará la reproducción_ de ese himno
    - `KeyL`: Cuando un resultado de búsqueda este subrayado con azul _iniciará la vista de letra_ de ese himno
    - `KeyP`: Cuando un resultado de búsqueda este subrayado con azul _agregará_ ese himno a la _lista de reproducción_
    - `KeyR`: Cuando un resultado de búsqueda este subrayado con azul _quitará_ ese himno a la _lista de reproducción_

- La app en la vista de reproducción de himno en video cuenta con los siguientes accesos directos de teclado:
    - `Escape | Backspace`: _Regresa_ a la vista de _búsqueda de himnos_
    - `Space`: Controla el estado de reproducción _(play | pause)_
    - `KeyM`: Cambia el estado de _mute_ del audio en el himno actual
    - `KeyC`: Cambia el _audio actual_ del himno en reproducción _(cantado | música)_
    - `ArrowLeft`: _Retrocede 10 segundos_ a la reproducción del himno actual
    - `ArrowRight`: _Avanza 10 segundos_ a la reproducción del himno actual
    - `KeyP`: _Muestra u oculta_ la lista de reproducción actual
<br/>

## Actualización (v2.0.3 -> v2.1.0)
La actualización incluye los siguientes cambios:
- Se agregó la letra de los himnos como imágenes
- Se mejoraron los controles de navegación
- Se agregaron accesos directos del teclado
- Se actualizaron las vistas de la app
- El icono de la app cambia de acuerdo al tema de Windows _(oscuro, claro)_

## Instalación
<!--
> [!IMPORTANT]
> **LINUX**

Para realizar la instalación en un equipo con Linux, este debe ser basado en **DEBIAN** y ejecutar el siguiente comando en la terminal:
```sh
sudo apt install ./rv60bible_1.1.1_amd64.deb
```
Una vez concluida la instalación, la aplicación estará disponible para su uso.
<br/><br/>
-->
> [!IMPORTANT]
> **WINDOWS**

1. Para realizar la instalación en un equipo con Windows primero deberás descargar los tres (6) archivos comprimidos y guardarlos en una misma carpeta:
```
HimnarioASD_2.1.0_standalone.7z.001
HimnarioASD_2.1.0_standalone.7z.002
HimnarioASD_2.1.0_standalone.7z.003
HimnarioASD_2.1.0_standalone.7z.004
HimnarioASD_2.1.0_standalone.7z.005
HimnarioASD_2.1.0_standalone.7z.006
```
2. Lo siguiente es descargar el programa **7-zip** para descomprimir los archivos, puedes descargarlo desde su sitio oficial [7-zip](https://www.7-zip.org/) o dar clic [aquí](https://github.com/ip7z/7zip/releases/download/26.01/7z2601-x64.exe)
3. Una vez tengas descargado el instalador del programa **7-zip** deberás instalarlo en tu PC
4. Abre el explorador de archivos en la carpeta donde descargaste los archivos de paso 1. localiza el archivo:
```
HimnarioASD_2.1.0_standalone.7z.001
```
5. Da clic derecho sobre él y selecciona alguna de las siguientes opciones en el menú desplegable:
```
"7-Zip" > "Extraer aquí"
"7-Zip" > "Extraer a HimnarioASD_2.1.0_standalone\" 
```
6. Dentro de la carpeta extraída, localiza el archivo:
```
himnarioasd.exe
```
7. Da doble clic para iniciar el **Himnario ASD (v2.1.0)**
8. Puedes mover la carpeta extraída a donde prefieras, la aplicación **Himnario ASD (v2.1.0)** es portable en su propia carpeta
<br/>

## Contacto
- Sitio web de desarrollo: [Serial30](https://serial30here.firebaseapp.com)
- Puede dejar comentarios en la página del proyecto: [Himnario ASD](https://serial30here.firebaseapp.com/projects/himnario-asd)
<br/>


